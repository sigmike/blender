...
#endif
{
  ...
  /* Create cursor model. */
  vr_cursor_model = Model::create(
      ui_cursor_obj_verts, ui_cursor_obj_nrmls, ui_cursor_obj_uvs, ui_cursor_obj_numverts);
...
int VR_Draw::create_controller_models(VR_UI_Type type)
{
  ...
  if (type == VR_UI_TYPE_OCULUS) {
    controller_model[VR_SIDE_LEFT] = Model::create(ui_oculus_touch_left_obj_verts,
                                                   ui_oculus_touch_left_obj_nrmls,
                                                   ui_oculus_touch_left_obj_uvs,
                                                   ui_oculus_touch_left_obj_numverts);
    controller_model[VR_SIDE_RIGHT] = Model::create(ui_oculus_touch_right_obj_verts,
                                                    ui_oculus_touch_right_obj_nrmls,
                                                    ui_oculus_touch_right_obj_uvs,
                                                    ui_oculus_touch_right_obj_numverts);
    controller_tex = new Texture(ui_oculus_touch_png);
  }
  else if (type == VR_UI_TYPE_VIVE) {
    controller_model[VR_SIDE_LEFT] = Model::create(ui_vive_controller_obj_verts,
                                                   ui_vive_controller_obj_nrmls,
                                                   ui_vive_controller_obj_uvs,
                                                   ui_vive_controller_obj_numverts);
    controller_model[VR_SIDE_RIGHT] = Model::create(ui_vive_controller_obj_verts,
                                                    ui_vive_controller_obj_nrmls,
                                                    ui_vive_controller_obj_uvs,
                                                    ui_vive_controller_obj_numverts);
    ...
    controller_model[VR_SIDE_LEFT] = Model::create(ui_microsoft_controller_left_obj_verts,
                                                   ui_microsoft_controller_left_obj_nrmls,
                                                   ui_microsoft_controller_left_obj_uvs,
                                                   ui_microsoft_controller_left_obj_numverts);
    controller_model[VR_SIDE_RIGHT] = Model::create(ui_microsoft_controller_right_obj_verts,
                                                    ui_microsoft_controller_right_obj_nrmls,
                                                    ui_microsoft_controller_right_obj_uvs,
                                                    ui_microsoft_controller_right_obj_numverts);
...
static void Util_Image_decodePNGRGBA_readData(png_structp png_ptr,
                                              png_bytep data,
                                              png_uint_32 length)
{
...
// clang-format off
const char *const VR_Draw::Shader::shader_col_vsource(STRING(
    #version 120\n attribute vec3 position; varying vec4 front_color; uniform mat4 modelview;
    uniform mat4 projection;
    void main() { gl_Position = projection * modelview * vec4(position, 1.0); }));

const char *const VR_Draw::Shader::shader_col_fsource(
...
// clang-format off
const char *const VR_Draw::Shader::shader_tex_vsource(
    STRING(#version 120\n attribute vec3 position; attribute vec3 normal; attribute vec2 uv;
           ...
           void main() {
             ...
             texcoord = uv;
             normal_transformed =
                 normalize(normal_matrix * vec4(normal, 0.0)).xyz; /* transformed to eye-space */
           }));

const char *const VR_Draw::Shader::shader_tex_fsource(STRING(
    #version 120\n varying vec3 normal_transformed; varying vec2 texcoord; uniform sampler2D tex;
    uniform vec4 color;
    void main() {
...
VR_Draw::Texture::Texture(const uchar *png_blob) : png_blob(png_blob), implementation(0)
{
  //
}
...
VR_Draw::Model::Model() : verts(0), nrmls(0), uvs(0), vertex_array(0), num_verts(0)
{
  this->texture = 0;
}
...
VR_Draw::Model *VR_Draw::Model::create(float *verts, float *nrmls, float *uvs, uint num_verts)
{
  ...
  glBindBuffer(GL_ARRAY_BUFFER, m->verts);
  glVertexAttribPointer(VR_Draw::Shader::shader_tex.position_location,
                        3,
                        GL_FLOAT,
                        GL_FALSE,
                        sizeof(float) * 3,
                        (void *)0);
  glBindBuffer(GL_ARRAY_BUFFER, m->nrmls);
  glVertexAttribPointer(VR_Draw::Shader::shader_tex.normal_location,
                        3,
                        GL_FLOAT,
                        GL_FALSE,
                        sizeof(float) * 3,
                        (void *)0);
  glBindBuffer(GL_ARRAY_BUFFER, m->uvs);
  glVertexAttribPointer(VR_Draw::Shader::shader_tex.uv_location,
                        2,
                        GL_FLOAT,
                        GL_FALSE,
                        sizeof(float) * 2,
...
int VR_Draw::Model::render()
{
  ...
  /* Load uniforms */
  glUniformMatrix4fv(VR_Draw::Shader::shader_tex.modelview_location,
                     1,
                     false,
                     (float *)VR_Draw::modelview_matrix.m);
  glUniformMatrix4fv(VR_Draw::Shader::shader_tex.projection_location,
                     1,
                     false,
                     (float *)VR_Draw::projection_matrix.m);
  glUniformMatrix4fv(VR_Draw::Shader::shader_tex.normal_matrix_location,
                     1,
                     true,
                     (float *)VR_Draw::modelview_matrix_inv.m);
...
void VR_Draw::render_rect(
    float left, float right, float top, float bottom, float z, float u, float v, Texture *tex)
{
  /* Save previous OpenGL state */
  GLint prior_program;
  glGetIntegerv(GL_CURRENT_PROGRAM, &prior_program);
  ...
  if (tex) {
    ...
    glUniformMatrix4fv(VR_Draw::Shader::shader_col.modelview_location,
                       1,
                       false,
                       (float *)VR_Draw::modelview_matrix.m);
    glUniformMatrix4fv(VR_Draw::Shader::shader_col.projection_location,
                       1,
                       false,
                       (float *)VR_Draw::projection_matrix.m);
  ...
  vertex_data[0][0] = left;
  vertex_data[0][1] = bottom;
  vertex_data[0][2] = z;
  vertex_data[1][0] = right;
  vertex_data[1][1] = bottom;
  vertex_data[1][2] = z;
  vertex_data[2][0] = left;
  vertex_data[2][1] = top;
  vertex_data[2][2] = z;
  vertex_data[3][0] = right;
  vertex_data[3][1] = top;
  vertex_data[3][2] = z;
  ...
  if (tex) {
    ...
    glVertexAttribPointer(VR_Draw::Shader::shader_col.position_location,
                          3,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(float) * 3,
...
void VR_Draw::render_frame(float left, float right, float top, float bottom, float b, float z)
{
  ...
  vertex_data[0][0] = left - b;
  vertex_data[0][1] = top + b;
  vertex_data[0][2] = z;
  vertex_data[1][0] = left;
  vertex_data[1][1] = top;
  vertex_data[1][2] = z;
  vertex_data[2][0] = right + b;
  vertex_data[2][1] = top + b;
  vertex_data[2][2] = z;
  vertex_data[2][0] = right + b;
  vertex_data[2][1] = top + b;
  vertex_data[2][2] = z;
  ...
  vertex_data[3][1] = top;
  vertex_data[3][2] = z;
  vertex_data[4][0] = right + b;
  vertex_data[4][1] = bottom - b;
  vertex_data[4][2] = z;
  vertex_data[5][0] = right;
  vertex_data[5][1] = bottom;
  vertex_data[5][2] = z;
  vertex_data[6][0] = left - b;
  vertex_data[6][1] = bottom - b;
  vertex_data[6][2] = z;
  vertex_data[7][0] = left;
  vertex_data[7][1] = bottom;
  vertex_data[7][2] = z;
  vertex_data[8][0] = left - b;
  vertex_data[8][1] = top + b;
  vertex_data[8][2] = z;
  vertex_data[9][0] = left;
  vertex_data[9][1] = top;
  vertex_data[9][2] = z;
...
void VR_Draw::render_box(const Coord3Df &p0, const Coord3Df &p1, bool outline)
{
  ...
  vertex_data[0][0] = p0.x;
  vertex_data[0][1] = p0.y;
  vertex_data[0][2] = p0.z;
  vertex_data[1][0] = p0.x;
  vertex_data[1][1] = p0.y;
  vertex_data[1][2] = p1.z;
  vertex_data[2][0] = p0.x;
  vertex_data[2][1] = p1.y;
  vertex_data[2][2] = p0.z;
  vertex_data[3][0] = p0.x;
  vertex_data[3][1] = p1.y;
  vertex_data[3][2] = p1.z;
  vertex_data[4][0] = p1.x;
  vertex_data[4][1] = p1.y;
  vertex_data[4][2] = p1.z;
  vertex_data[5][0] = p0.x;
  vertex_data[5][1] = p0.y;
  vertex_data[5][2] = p1.z;
  vertex_data[6][0] = p1.x;
  vertex_data[6][1] = p0.y;
  vertex_data[6][2] = p1.z;
  vertex_data[7][0] = p0.x;
  vertex_data[7][1] = p0.y;
  vertex_data[7][2] = p0.z;
  vertex_data[8][0] = p1.x;
  vertex_data[8][1] = p0.y;
  vertex_data[8][2] = p0.z;
  vertex_data[9][0] = p0.x;
  vertex_data[9][1] = p1.y;
  vertex_data[9][2] = p0.z;
  vertex_data[10][0] = p1.x;
  vertex_data[10][1] = p1.y;
  vertex_data[10][2] = p0.z;
  vertex_data[11][0] = p1.x;
  vertex_data[11][1] = p1.y;
  vertex_data[11][2] = p1.z;
  vertex_data[12][0] = p1.x;
  vertex_data[12][1] = p0.y;
  vertex_data[12][2] = p0.z;
  vertex_data[13][0] = p1.x;
  vertex_data[13][1] = p0.y;
  vertex_data[13][2] = p1.z;
  ...
  if (outline) {
    ...
    line_vertex_data[0][0] = p0.x;
    line_vertex_data[0][1] = p0.y;
    line_vertex_data[0][2] = p0.z;
    line_vertex_data[1][0] = p0.x;
    line_vertex_data[1][1] = p1.y;
    line_vertex_data[1][2] = p0.z;
    line_vertex_data[2][0] = p1.x;
    line_vertex_data[2][1] = p1.y;
    line_vertex_data[2][2] = p0.z;
    line_vertex_data[3][0] = p1.x;
    line_vertex_data[3][1] = p0.y;
    line_vertex_data[3][2] = p0.z;
    line_vertex_data[4][0] = p0.x;
    line_vertex_data[4][1] = p0.y;
    line_vertex_data[4][2] = p0.z;
    line_vertex_data[5][0] = p0.x;
    line_vertex_data[5][1] = p0.y;
    line_vertex_data[5][2] = p1.z;
    line_vertex_data[6][0] = p0.x;
    line_vertex_data[6][1] = p1.y;
    line_vertex_data[6][2] = p1.z;
    line_vertex_data[7][0] = p0.x;
    line_vertex_data[7][1] = p1.y;
    line_vertex_data[7][2] = p0.z;
    line_vertex_data[8][0] = p0.x;
    line_vertex_data[8][1] = p1.y;
    line_vertex_data[8][2] = p1.z;
    line_vertex_data[9][0] = p1.x;
    line_vertex_data[9][1] = p1.y;
    line_vertex_data[9][2] = p1.z;
    line_vertex_data[10][0] = p1.x;
    line_vertex_data[10][1] = p1.y;
    line_vertex_data[10][2] = p0.z;
    line_vertex_data[11][0] = p1.x;
    line_vertex_data[11][1] = p1.y;
    line_vertex_data[11][2] = p1.z;
    line_vertex_data[12][0] = p1.x;
    line_vertex_data[12][1] = p0.y;
    line_vertex_data[12][2] = p1.z;
    line_vertex_data[13][0] = p1.x;
    line_vertex_data[13][1] = p0.y;
    line_vertex_data[13][2] = p0.z;
    line_vertex_data[14][0] = p1.x;
    line_vertex_data[14][1] = p0.y;
    line_vertex_data[14][2] = p1.z;
    line_vertex_data[15][0] = p0.x;
    line_vertex_data[15][1] = p0.y;
    line_vertex_data[15][2] = p1.z;
...
void VR_Draw::render_ball(float r, bool golf)
{
  ...
  for (int x = 0; x < res; ++x) {
    for (int y = -res / 2; y < res / 2; ++y) {
      ...
      vertex_data[i][0] = r1 * x1;
      vertex_data[i][1] = r1 * y1;
      vertex_data[i][2] = z1;
      ++i;
      vertex_data[i][0] = r1 * x2;
      vertex_data[i][1] = r1 * y2;
      vertex_data[i][2] = z1;
      ++i;
      vertex_data[i][0] = r2 * x2;
      vertex_data[i][1] = r2 * y2;
      vertex_data[i][2] = z2;
      ++i;
      ...
      vertex_data[i][2] = z2;
      ++i;
      vertex_data[i][0] = r2 * x1;
      vertex_data[i][1] = r2 * y1;
      vertex_data[i][2] = z2;
      ++i;
    }
  }
...
void VR_Draw::render_arrow(const Coord3Df &from, const Coord3Df &to, float width)
{
  ...
  static GLfloat vertex_data[4][3];
  vertex_data[0][0] = vd.x + from.x;
  vertex_data[0][1] = vd.y + from.y;
  vertex_data[0][2] = to.z;
  vertex_data[1][0] = vn.y + from.y;
  vertex_data[1][1] = -vn.x + from.y;
  vertex_data[1][2] = from.z;
  vertex_data[2][0] = -vn.y + from.x;
  vertex_data[2][1] = vn.x + from.y;
  vertex_data[2][2] = from.z;
  vertex_data[3][0] = -vn.x + from.x;
  vertex_data[3][1] = -vn.y + from.y;
  vertex_data[3][2] = from.z;
...
void VR_Draw::render_string(const char *str,
                            float w,
                            float h,
                            VR_HAlign h_align,
                            VR_VAlign v_align,
                            ...
                            float z_offset)
{
  ...
  while (str[i]) {
    ...
    vertex_data[0][0] = x;
    vertex_data[0][1] = y - h;
    vertex_data[0][2] = z_offset; /* bottom-left */
    vertex_data[1][0] = x + w;
    vertex_data[1][1] = y - h;
    vertex_data[1][2] = z_offset; /* bottom-right */
    vertex_data[2][0] = x;
    vertex_data[2][1] = y;
    vertex_data[2][2] = z_offset; /* top-left */
    vertex_data[3][0] = x + w;
    vertex_data[3][1] = y;
    vertex_data[3][2] = z_offset; /* top-right */
    ...
    glBindBuffer(GL_ARRAY_BUFFER, verts);
    glBufferData(
        GL_ARRAY_BUFFER, 3 * 4 * sizeof(float), vertex_data, GL_STATIC_DRAW);  // GL_DYNAMIC_DRAW);
    glVertexAttribPointer(VR_Draw::Shader::shader_tex.position_location,
                          3,
                          GL_FLOAT,
                          GL_FALSE,
    ...
    /* Create uv buffers */
    static GLfloat uv_data[4][2];
    uv_data[0][0] = float(col + 0) / 14.0f;
    uv_data[0][1] = float(row + 1) / 7.0f;
    uv_data[1][0] = float(col + 1) / 14.0f;
    uv_data[1][1] = float(row + 1) / 7.0f;
    uv_data[2][0] = float(col + 0) / 14.0f, uv_data[2][1] = float(row + 0) / 7.0f;
    uv_data[3][0] = float(col + 1) / 14.0f;
    uv_data[3][1] = float(row + 0) / 7.0f;
