    ...
    Main *bmain, Scene *scene, ViewLayer *view_layer, Base *base_old, BMesh *bm_old)
{
  ...
  bm_new = BM_mesh_create(&bm_mesh_allocsize_default, &params);
  ...
  /* DAG_relations_tag_update(bmain); */ /* normally would call directly after but in this case
                                            delay recalc */
  assign_matarar(
      bmain, base_new->object, give_matarar(obedit), *give_totcolp(obedit)); /* new in 2.5 */
    ...
    for (ctx_link = (CollectionPointerLink *)ctx_data_list.first; ctx_link;
      ...
      if (ob->type == OB_MESH) {
        ...
        if (!ID_IS_LINKED(me)) {
          ...
          BMeshCreateParams params = {0};
          params.use_toolflags = true;
          bm_old = BM_mesh_create(&bm_mesh_allocsize_default, &params);
          ...
          if (retval_iter) {
            BM_mesh_bm_to_me(bmain, bm_old, me, &mtm_params);
...
void Widget_Separate::render_icon(const Mat44f &t,
