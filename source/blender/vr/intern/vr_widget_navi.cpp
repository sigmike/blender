...
void Widget_Navi::GrabAir::drag_contd(VR_UI::Cursor &c)
{
  ...
  if (c.bimanual) {
    ...
    float curr_s = sqrt(((curr_h.m[3][0] - curr_o.m[3][0]) * (curr_h.m[3][0] - curr_o.m[3][0])) +
                        ((curr_h.m[3][1]) - curr_o.m[3][1]) * (curr_h.m[3][1] - curr_o.m[3][1])) +
                   ((curr_h.m[3][2] - curr_o.m[3][2]) * (curr_h.m[3][2] - curr_o.m[3][2]));
    float start_s = sqrt(((prev_h.m[3][0] - prev_o.m[3][0]) * (prev_h.m[3][0] - prev_o.m[3][0])) +
  ...
  if (VR_UI::ctrl_key_get() || Widget_Navi::nav_lock[1]) {
    ...
    switch (Widget_Navi::nav_lock[1]) {
      ...
      default: {
        ...
        float rot_ident[3][4] = {
            {prev_scale, 0, 0, 0}, {0, prev_scale, 0, 0}, {0, 0, prev_scale, 0}};
  ...
  if (Widget_Navi::nav_lock[0]) {
    ...
    switch (Widget_Navi::nav_lock[0]) {
      case VR_UI::NAVLOCK_TRANS_UP: {
        ...
        curr = VR_UI::convert_space(
            curr, VR_SPACE_BLENDER, VR_SPACE_REAL); /* locked in real-world coordinates */
        curr.m[3][2] = prev.m[3][2];
        prev = VR_UI::convert_space(prev, VR_SPACE_REAL, VR_SPACE_BLENDER);
        curr = VR_UI::convert_space(
            curr, VR_SPACE_REAL, VR_SPACE_BLENDER); /* revert to Blender coordinates */
        break;
      }
      ...
      default: {
        prev = VR_UI::convert_space(prev, VR_SPACE_BLENDER, VR_SPACE_REAL);
        curr = VR_UI::convert_space(
            curr, VR_SPACE_BLENDER, VR_SPACE_REAL); /* locked in real-world coodinates */
...
void Widget_Navi::GrabAir::render_icon(const Mat44f &t,
                                       ...
                                       bool touched)
{
  ...
  VR_Draw::render_rect(
      -0.009f, 0.009f, 0.009f, -0.009f, 0.001f, 1.0f, 1.0f, VR_Draw::nav_grabair_tex);
}
...
void Widget_Navi::Joystick::drag_contd(VR_UI::Cursor &c)
{
  ...
  if (vr_get_obj()->ui_type == VR_UI_TYPE_FOVE) {
    ...
    if (VR_UI::cursor_offset_enabled) {
      /* Maybe we actually want to use the cursor position instead of the controller (gaze
       * convergence) position, but for now disable it because it makes joystick navigation
       * difficult. */
      v = *(Coord3Df *)(vr_get_obj()->t_controller[VR_SPACE_REAL][VR_SIDE_MONO][3]) -
          *(Coord3Df *)hmd.m[3];
    }
    else {
    ...
    if (a < 0.36f * PI) {
      ...
      delta.m[1][0] = sin_a;
      delta.m[0][1] = -sin_a;
      delta.m[3][0] += cos_a * hmd.m[3][0] - sin_a * hmd.m[3][1] -
                       hmd.m[3][0]; /* rotate around HMD/POV: */
      delta.m[3][1] += cos_a * hmd.m[3][1] + sin_a * hmd.m[3][0] -
                       hmd.m[3][1]; /* use HMD position as rotation pivot */
      delta.m[2][2] = 1;
      delta.m[3][3] = 1;
    }
    else if (a > 0.64f * PI) {
...
void Widget_Navi::Joystick::render_icon(const Mat44f &t,
                                        VR_Side controller_side,
                                        bool active,
                                        bool touched)
{
  ...
  VR_Draw::render_rect(
      -0.009f, 0.009f, 0.009f, -0.009f, 0.001f, 1.0f, 1.0f, VR_Draw::nav_joystick_tex);
}
...
void Widget_Navi::Teleport::render_icon(const Mat44f &t,
                                        VR_Side controller_side,
                                        bool active,
                                        bool touched)
{
  ...
  VR_Draw::render_rect(
      -0.009f, 0.009f, 0.009f, -0.009f, 0.001f, 1.0f, 1.0f, VR_Draw::nav_teleport_tex);
}
