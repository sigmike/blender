...
class VR_Widget_Layout {
 public:
  ...
  typedef enum ButtonBit : uint64_t {
    ...
    ,
    BUTTONBIT_RIGHTTRIGGER = (uint64_t(1)
                              << 1) /* Bit of the right controller trigger as a button */
    ,
    BUTTONBITS_TRIGGERS =
        (BUTTONBIT_LEFTTRIGGER |
         BUTTONBIT_RIGHTTRIGGER) /* Bits of pressing any of the controller triggers */
    ,
    ...
    BUTTONBITS_GRIPS =
        (BUTTONBIT_LEFTGRIP |
         BUTTONBIT_RIGHTGRIP) /* Bit of pressing any of the "grip" (side/shoulder) buttons  */
    ,
    ...
    ,
    BUTTONBITS_DPADANY = (BUTTONBIT_DPADLEFT | BUTTONBIT_DPADRIGHT | BUTTONBIT_DPADUP |
                          BUTTONBIT_DPADDOWN) /* Bits of pushing the trackpad in any direction */
    ,
    BUTTONBIT_LEFTDPAD = (uint64_t(1)
                          << 8) /* Bit of pressing the left trackpad (center) as a button */
    ,
    BUTTONBIT_RIGHTDPAD = (uint64_t(1)
                           << 9) /* Bit of pressing the right trackpad (center) as a button */
    ,
    BUTTONBITS_DPADS = (BUTTONBIT_LEFTDPAD |
                        BUTTONBIT_RIGHTDPAD) /* Bits of pressing any of the trackpads */
    ,
    ...
    ,
    BUTTONBITS_STICKANY =
        (BUTTONBIT_STICKLEFT | BUTTONBIT_STICKRIGHT | BUTTONBIT_STICKUP |
         BUTTONBIT_STICKDOWN) /* Bits of pushing the thumbstick in any direction */
    ,
    ...
    ,
    BUTTONBITS_STICKS = (BUTTONBIT_LEFTSTICK |
                         BUTTONBIT_RIGHTSTICK) /* Bits of pressing any of the thumbsticks */
    ,
    BUTTONBIT_LEFTTHUMBREST = (uint64_t(1)
                               << 16) /* Bit of pressing (touching) the left thumbrest area */
    ,
    BUTTONBIT_RIGHTTHUMBREST = (uint64_t(1)
                                << 17) /* Bit of pressing (touching) the right thumbrest area */
    ,
    BUTTONBITS_THUMBRESTS =
        (BUTTONBIT_LEFTTHUMBREST |
         BUTTONBIT_RIGHTTHUMBREST) /* Bit of pressing (touching) any of the thumbrest areas */
    ,
  ...
  static ButtonID buttonBitToID(
      ButtonBit bit); /* Helper function to convert button bits to button IDs. */
  static ButtonBit buttonIDToBit(
      ButtonID id); /* Helper function to convert button IDs to button bit. */
  static std::string buttonIDToString(
      ButtonID id); /* Helper function to convert button IDs to ident string. */
  static ButtonID buttonStringToID(
      const std::string str); /* Helper function to convert button ident strings to IDs. */

  static Coord3Df button_positions[VR_UI_TYPES][VR_SIDES]
                                  [BUTTONIDS]; /* 3D positions of the buttons with respect to the
                                                  controllers. */
 ...
 public:
  ...
  typedef struct Layout {
    ...
    VR_Widget
        *m[VR_SIDES][BUTTONIDS][VR_UI::ALTSTATES]; /* Mapping of buttons to widgets (one for each
                                                      controller side, each button, ALT on/off). */
    ButtonBit ctrl_button_bits[VR_SIDES][VR_UI::ALTSTATES]; /* Controller button bits defined to be
                                                               the 'ctrl buttons' (per side, and
                                                               whether ALT is pressed). */
    ButtonBit shift_button_bits[VR_SIDES][VR_UI::ALTSTATES]; /* Controller button bits defined to
                                                                be the 'shift buttons' (per side,
                                                                and whether ALT is pressed). */
    ButtonBit alt_button_bits[VR_SIDES]; /* Controller button bits defined to be the 'alt buttons'
                                            (per side). */
  } Layout;
  ...
  static VR_UI::Error get_current_layout(
      std::string &layout); /* Get the current UI layout (task mode). */
  static VR_UI::Error set_current_layout(
      const std::string &layout_name); /* Set the current UI layout (task mode). */
  static VR_UI::Error rename_current_layout(
      const std::string &new_name);            /* Update the name of the layout (task mode). */
  static VR_UI::Error delete_current_layout(); /* Delete the current layout (task mode). */
  static VR_UI::Error get_layouts_list(
      std::vector<std::string> &list); /* Get list of all known UI layouts (task modes). */
  static VR_UI::Error map_widget(VR_Side side,
                                 std::string event,
                                 VR_UI::AltState alt,
                                 std::string widget); /* Set a button-to-widget mapping. */
  static VR_UI::Error unmap_widget(const std::string &widget); /* Unmap widget everywhere. */
  static VR_UI::Error get_mapped_widget(
      VR_Side side,
      const std::string &event,
      VR_UI::AltState alt,
      std::string &widget); /* Get a single button-to-widget mapping (widget name). */
  static VR_UI::Error get_mapped_event(
      const std::string &widget,
      VR_Side &side,
      std::string &event,
      VR_UI::AltState &alt); /* Get a single button-to-widget mapping (event name). */
  static VR_UI::Error get_mapped_event(
      const VR_Widget *widget,
      VR_Side &side,
      std::string &event,
      VR_UI::AltState &alt); /* Get a single button-to-widget mapping (event name). */
  static VR_UI::Error get_mapped_event(
  ...
  static VR_UI::Error get_mapped_event(
      ...
      ButtonID &event,
      VR_UI::AltState &alt); /* Get a single button-to-widget mapping (event name). */
  static VR_UI::Error get_events_list(
      std::vector<std::string>
          &events); /* Get a list of all the events (buttons) available for this UI. */
};

inline VR_Widget_Layout::ButtonBit &operator|=(VR_Widget_Layout::ButtonBit &b0,
                                               const VR_Widget_Layout::ButtonBit &b1)
{
  return b0 = VR_Widget_Layout::ButtonBit(b0 | b1);
};
inline VR_Widget_Layout::ButtonBit operator|(const VR_Widget_Layout::ButtonBit &b0,
                                             const VR_Widget_Layout::ButtonBit &b1)
{
