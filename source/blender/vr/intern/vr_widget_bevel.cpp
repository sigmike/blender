...
typedef struct {
  ...
  float shift_value[NUM_VALUE_KINDS]; /* The current value when shift is pressed. Negative when
                                         shift not active. */
  bool is_modal;
...
static void bevel_harden_normals(BMEditMesh *em, BMOperator *bmop, float face_strength)
{
  ...
  BMOpSlot *nslot = BMO_slot_get(bmop->slots_out,
                                 "normals.out"); /* Per vertex normals depending on hn_mode */
    ...
    do {
      if ((BM_elem_flag_test(l_cur->v, BM_ELEM_SELECT)) &&
        ...
        if (!BM_elem_flag_test(l_cur->e, BM_ELEM_TAG) &&
            !BM_elem_flag_test(l_cur->prev->e, BM_ELEM_TAG)) {
...
static bool edbm_bevel_init(bContext *C, wmOperator *op, const bool is_modal)
{
...

  {
    ...
    opdata->ob_store = (BevelObjectStore *)MEM_malloc_arrayN(
