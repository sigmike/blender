...
void Widget_Knife::drag_start(VR_UI::Cursor &c)
{
  ...
  /* The knifetool installs an eventhandler for modal that we don't need or
   * want. Find it and remove it... */
  wmWindow *win = CTX_wm_window(C);
  wmEventHandler *handler_base = (wmEventHandler *)win->modalhandlers.first;
...
//Object *obedit = CTX_data_edit_object(C);
  ...
  float (*mval_fl)[2] = (float(*)[2])MEM_mallocN(sizeof(float)*2*2, "BlenderXR Knife Tool tmp
screencoords");
