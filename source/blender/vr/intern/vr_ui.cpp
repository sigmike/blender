...
ui64 VR_t_now(
    0); /* Current (most recent) timestamp. This will be updated (1) when updating tracking (2)
           when starting rendering a new frame (3) before executing UI operations. */
...
static ui64 currentSystemTime()
{
  ...
  SYSTEMTIME t;
  GetSystemTime(&t);
  return (((((ui64)(t.wDay) * 24 + t.wHour) * 60 + t.wMinute) * 60 + t.wSecond) * 1000 +
          t.wMilliseconds);
#else
  timespec ts;
...
VR_UI::Cursor VR_UI::cursor[VR_MAX_CONTROLLERS] = {
    VR_UI::Cursor(), VR_UI::Cursor(), VR_UI::Cursor()};
...
VR_Widget *VR_UI::get_current_tool(VR_Side side)
{
  ...
  for (int i = 0; i < VR_Widget_Layout::layouts[type].size(); ++i) {
    if (VR_Widget_Layout::layouts[type][i]->name == VR_Widget_Layout::current_layout->name) {
      ...
      return VR_Widget_Layout::layouts[type][i]
          ->m[side][VR_Widget_Layout::ButtonID::BUTTONID_TRIGGER][alt];
    }
  }
...
VR_UI::Error VR_UI::set_current_tool(const VR_Widget *tool, VR_Side side)
{
  ...
  for (int i = 0; i < VR_Widget_Layout::layouts[type].size(); ++i) {
    if (VR_Widget_Layout::layouts[type][i]->name == VR_Widget_Layout::current_layout->name) {
      /* The currently active tool is the one mapped to the controller trigger. */
      VR_Widget_Layout::layouts[type][i]
          ->m[side][VR_Widget_Layout::ButtonID::BUTTONID_TRIGGER][alt] = (VR_Widget *)tool;
      break;
    }
  }
...
const Mat44f &VR_UI::hmd_position_get(VR_Space space, bool inverse)
{
  ...
  if (space == VR_SPACE_REAL) {
    ...
    if (!VR_UI::hmd_position_current[VR_SPACE_BLENDER][0]) {
      _va_mul_m4_series_3(
          vr->t_hmd[VR_SPACE_BLENDER], vr->t_hmd[VR_SPACE_REAL], VR_UI::navigation_matrix.m);
...
const Mat44f &VR_UI::eye_position_get(VR_Space space, VR_Side side, bool inverse)
{
  ...
  if (space == VR_SPACE_REAL) {
    ...
    if (!VR_UI::eye_position_current[VR_SPACE_BLENDER][side][0]) {
      _va_mul_m4_series_3(vr->t_eye[VR_SPACE_BLENDER][side],
                          vr->t_eye[VR_SPACE_REAL][side],
                          VR_UI::navigation_matrix.m);
  ...
  if (space == VR_SPACE_REAL) {
    if (inverse) {
      if (!VR_UI::controller_position_current[VR_SPACE_REAL][side][1]) {
        invert_m4_m4(vr->t_controller_inv[VR_SPACE_REAL][side],
                     vr->t_controller[VR_SPACE_REAL][side]);
    ...
    if (!VR_UI::controller_position_current[VR_SPACE_BLENDER][side][0]) {
      _va_mul_m4_series_3(vr->t_controller[VR_SPACE_BLENDER][side],
                          vr->t_controller[VR_SPACE_REAL][side],
                          VR_UI::navigation_matrix.m);
    ...
    if (inverse) {
      if (!VR_UI::controller_position_current[VR_SPACE_BLENDER][side][1]) {
        invert_m4_m4(vr->t_controller_inv[VR_SPACE_BLENDER][side],
                     vr->t_controller[VR_SPACE_BLENDER][side]);
...
Coord3Df VR_UI::convert_space(const Coord3Df &v, VR_Space v_space, VR_Space target_space)
{
  ...
  Coord3Df ret(v.x * m.m[0][0] + v.y * m.m[1][0] + v.z * m.m[2][0] + m.m[3][0],
...
VR_UI::Error VR_UI::update_tracking()
{
  ...
  for (int i = 0; i < VR_SIDES; ++i) {
    ...
    VR_UI::eye_position_current[VR_SPACE_REAL][i][1] =
        true; /* Will be calculated each frame in vr_update_view_matrix().. */
    VR_UI::eye_position_current[VR_SPACE_BLENDER][i][0] =
        true; /* Will be calculated each frame in vr_compute_viewmat(). */
...
VR_UI::Error VR_UI::execute_operations()
{
  ...
  if (action_update_dt < VR_UI_MAXUPDATEINTERVAL && VR_UI::fps_render) {
    ...
    const ui64 max_fps = 1000 / VR_UI_MINUPDATEINTERVAL;
          const float render_ratio = float(VR_UI::fps_render - min_fps) / (60.0f
  - float(min_fps)); const float target_fps = (render_ratio * float(max_fps -
  min_fps)) + min_fps; const ui64 target_interval = ui64(1000.0f / target_fps);
  ...
  if (action_update_dt < VR_UI_MAXUPDATEINTERVAL) {
    // Linearly degrade update frequency if rendering framerate drops below
    // 60fps, hitting the minimum at 30fps
    const ui64 min_fps = 1000 / VR_UI_MAXUPDATEINTERVAL;
    const ui64 max_fps = 1000 / VR_UI_MINUPDATEINTERVAL;
    const float curr_fps = (float)VR_UI::fps_render;  // fps estimate from the last frame
    const float taget_fps = (curr_fps > 30.0f) ? (((curr_fps - 30.0f) / (60.0f - 30.0f)) *
    ...
    if (vr->controller[VR_SIDE_RIGHT]->available) {
      ...
      if (VR_UI::ui_type == VR_UI_TYPE_FOVE) {
        ...
        if (VR_UI::cursor[VR_SIDE_LEFT].last_buttons & VR_Widget_Layout::BUTTONBITS_GRIPS) {
          VR_UI::cursor[VR_SIDE_RIGHT].position.position[VR_SPACE_REAL].mat =
              VR_UI::cursor[VR_SIDE_LEFT].position.position[VR_SPACE_REAL].mat;
          vr->controller[VR_SIDE_RIGHT]->buttons = VR_Widget_Layout::BUTTONBITS_GRIPS;
          VR_UI::update_cursor(VR_UI::cursor[VR_SIDE_RIGHT]);
          VR_UI::cursor[VR_SIDE_RIGHT].last_position.position[VR_SPACE_REAL].mat =
              VR_UI::cursor[VR_SIDE_LEFT].position.position[VR_SPACE_REAL].mat;
  ...
  VR_UI::ctrl_key = CtrlState(VR_UI::cursor[VR_SIDE_LEFT].ctrl |
                              VR_UI::cursor[VR_SIDE_RIGHT].ctrl);
  VR_UI::shift_key = ShiftState(VR_UI::cursor[VR_SIDE_LEFT].shift |
                                VR_UI::cursor[VR_SIDE_RIGHT].shift);
...
VR_UI::Error VR_UI::update_cursor(Cursor &c)
{
  ...
  if (c.trigger) {
    ...
    else if (Widget_Menu::menu_type[c.side] != VR_Widget::MENUTYPE_AS_NAVI &&
             (c.last_buttons & VR_Widget_Layout::BUTTONBITS_TRIGGERS)) {
  ...
  if (VR_UI::pie_menu_active[c.side]) {
    ...
    bool touched = ((buttons_touched & (VR_UI::ui_type == VR_UI_TYPE_VIVE ?
                                            VR_Widget_Layout::BUTTONBITS_DPADANY :
                                            VR_Widget_Layout::BUTTONBITS_STICKANY)) != 0);
    bool stick_pressed = ((buttons & (VR_UI::ui_type == VR_UI_TYPE_OCULUS ?
                                          VR_Widget_Layout::BUTTONBITS_STICKS :
                                          VR_Widget_Layout::BUTTONBITS_DPADS)) != 0);
    ...
    if (menu) {
      ...
      buttons &= ~(VR_UI::ui_type == VR_UI_TYPE_VIVE ? VR_Widget_Layout::BUTTONBITS_DPADANY :
                                                       VR_Widget_Layout::BUTTONBITS_STICKANY);
      buttons &= ~(VR_UI::ui_type == VR_UI_TYPE_OCULUS ? VR_Widget_Layout::BUTTONBITS_STICKS :
  ...
  if (c.interaction_state == VR_UI::BUTTONSTATE_IDLE) {
    ...
    VR_Widget_Layout::ButtonBit dead_button_bits = VR_Widget_Layout::ButtonBit(
        VR_Widget_Layout::current_layout->ctrl_button_bits[c.side][VR_UI::alt_key] |
        VR_Widget_Layout::current_layout->shift_button_bits
            [c.side]
            [VR_UI::alt_key]);  // | VR_Widget_Layout::current_layout->alt_button_bits[c.side]);
    if ((buttons & (~dead_button_bits)) ==
        0) { /* no buttons pressed except shift/alt: nothing to do */
      return ERROR_NONE;
    }
    /* else: button hit */
    VR_Widget_Layout::ButtonID button_id = VR_Widget_Layout::buttonBitToID(
        VR_Widget_Layout::ButtonBit(buttons)); /* will get the main/dominant button */
    ...
    c.interaction_button = VR_Widget_Layout::buttonIDToBit(
        button_id); /* <- *only* the bits for the active button */
  ...
  if (c.interaction_state == VR_UI::BUTTONSTATE_DOWN) {
    ...
    VR_Widget_Layout::ButtonBit new_button = VR_Widget_Layout::ButtonBit(buttons &
                                                                         ~c.last_buttons);
    if (new_button) {
      ...
      if (new_button_id != VR_Widget_Layout::BUTTONID_UNKNOWN) {
        VR_Widget *new_button_widget =
            VR_Widget_Layout::current_layout->m[c.side][new_button_id][VR_UI::alt_key];
        if (new_button_widget) {
          if (!c.interaction_widget ||
              (c.interaction_widget->allows_focus_steal(new_button_widget->type()) &&
               new_button_widget->steals_focus(c.interaction_widget->type()))) {
            /* Focus steal */
            c.interaction_widget =
                VR_Widget_Layout::current_layout->m[c.side][new_button_id][VR_UI::alt_key];
            c.interaction_state = VR_UI::BUTTONSTATE_DOWN;
            c.interaction_time = now;
            c.interaction_position = c.position;
    ...
    if ((c.other_hand->interaction_state == VR_UI::BUTTONSTATE_DRAG &&
         c.interaction_widget == c.other_hand->interaction_widget) ||
      ...
      if (c.interaction_widget == c.other_hand->interaction_widget) {
        ...
        if (c.other_hand->interaction_state ==
            VR_UI::BUTTONSTATE_DRAG) {                 /* the other hand is already dragging */
    ...
    if (c.interaction_widget->has_drag(c) &&
        (now - c.interaction_time >= VR_UI::drag_threshold_time)) {
      /* Check for motion that would indicate a drag. */
      const Mat44f &pi = c.interaction_position.get();
      const Mat44f &pc = c.position.get();
      float d;
      if (c.interaction_widget->type() == VR_Widget::TYPE_ANNOTATE ||
          VR_Widget::TYPE_TRANSFORM || /* For annotation and transform widget, start dragging
                                          immediately. */
  ...
  if (c.interaction_state == VR_UI::BUTTONSTATE_DRAG) {
    ...
    if (new_button && (new_button_id = VR_Widget_Layout::buttonBitToID(new_button)) !=
                          VR_Widget_Layout::BUTTONID_UNKNOWN) {
      VR_Widget *new_button_widget =
          VR_Widget_Layout::current_layout->m[c.side][new_button_id][VR_UI::alt_key];
      if (new_button_widget) {
        if (!c.interaction_widget) {
        ...
        else if (c.interaction_widget->allows_focus_steal(new_button_widget->type()) &&
                 new_button_widget->steals_focus(c.interaction_widget->type())) {
          /* Focus steal
...
VR_UI::Error VR_UI::post_render(VR_Side side)
{
  ...
  if (VR_UI::ui_type == VR_UI_TYPE_FOVE) {
    ...
    bool render_left = VR_UI::cursor_active_get(VR_SIDE_LEFT) &&
                       VR_UI::cursor_visible_get(VR_SIDE_LEFT);
    bool render_right = VR_UI::cursor_active_get(VR_SIDE_RIGHT) &&
                        VR_UI::cursor_visible_get(VR_SIDE_RIGHT);
...
{
  if (controller_side ==
      VR_SIDE_BOTH) { /* Render both controllers in one function call (optimized). */
    ...
    if (VR_UI::ui_type == VR_UI_TYPE_MICROSOFT) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        VR_Draw::render_rect(
            -0.005f, 0.005f, 0.005f, -0.005f, 0.001f, 1.0f, 1.0f, VR_Draw::cursor_tex);
        VR_Draw::set_depth_test(true, true);
      }
    }
    else {
...
VR_UI::Error VR_UI::render_widget_icons(VR_Side controller_side, const Mat44f &t_controller)
{
  ...
  if (VR_UI::pie_menu_active[controller_side]) {
    ...
    /* Render pie menu. */
    VR_Widget_Layout::ButtonID btn = (VR_UI::ui_type == VR_UI_TYPE_VIVE ?
                                          VR_Widget_Layout::BUTTONID_DPAD :
                                          VR_Widget_Layout::BUTTONID_STICK);
    VR_Widget_Layout::ButtonBit btnbit = (VR_UI::ui_type == VR_UI_TYPE_VIVE ?
                                              VR_Widget_Layout::BUTTONBITS_DPADANY :
                                              VR_Widget_Layout::BUTTONBITS_STICKANY);
    *((Coord3Df *)t_icon.m[3]) =
        VR_Widget_Layout::button_positions[VR_UI::ui_type][controller_side][btn];
    VR_Draw::set_depth_test(true, false);
    if (VR_UI::ui_type == VR_UI_TYPE_MICROSOFT && !Widget_Menu::action_settings[controller_side]) {
      ...
      temp = t_controller;
      *(Coord3Df *)temp.m[3] += ((*(Coord3Df *)t_controller.m[1]).normalize() +
                                 (*(Coord3Df *)t_controller.m[2]).normalize()) *
                                0.01f;
    ...
    buttons &= ~(VR_UI::ui_type == VR_UI_TYPE_OCULUS ? VR_Widget_Layout::BUTTONBITS_STICKS :
                                                       VR_Widget_Layout::BUTTONBITS_DPADS);
    buttons_touched &= ~(VR_UI::ui_type == VR_UI_TYPE_VIVE ?
                             VR_Widget_Layout::BUTTONBITS_DPADANY :
                             VR_Widget_Layout::BUTTONBITS_STICKANY);
    buttons_touched &= ~(VR_UI::ui_type == VR_UI_TYPE_OCULUS ?
                             VR_Widget_Layout::BUTTONBITS_STICKS :
                             VR_Widget_Layout::BUTTONBITS_DPADS);
    ...
    if (Widget_Menu::action_settings[controller_side]) {
      for (VR_Widget_Layout::ButtonID btn = VR_Widget_Layout::ButtonID(0);
           btn < VR_Widget_Layout::BUTTONIDS;
           btn = VR_Widget_Layout::ButtonID(btn + 1)) {
      ...
      for (VR_Widget_Layout::ButtonID btn = VR_Widget_Layout::ButtonID(0);
        ...
        else if (btn == VR_Widget_Layout::BUTTONID_STICKLEFT &&
                 VR_UI::ui_type != VR_UI_TYPE_VIVE) {
    ...
    for (int controller_side = 0; controller_side < VR_SIDES; ++controller_side) {
      ...
      if (widget && widget->do_render[side]) {
        if ((widget->type() == VR_Widget::TYPE_TRANSFORM ||
             widget->type() == VR_Widget::TYPE_EXTRUDE) &&
            !manip_rendered) {
          /* Manipulator */
          VR_Draw::set_depth_test(false, false);
...
VR_UI::Error VR_UI::render_menus(const Mat44f *_model, const Mat44f *_view)
{
  ...
  VR_Draw::render_string(
      fps_str.c_str(), 0.03f, 0.03f, VR_HALIGN_CENTER, VR_VALIGN_TOP, 0.0f, 0.18f, 0.001f);
