...
class VR_Draw {
 ...
 protected:
  ...
  static void *drawable; /* Pointer to the GLX drawable (GLXDrawable*). Either an X window ID or a
                            GLX pixmap ID. */
  static void
      *context; /* Pointer to the GLX rendering context (GLXContext*) attached to drawable. */
  ...
  typedef struct Shader {
    ...
    int normal_matrix_location; /* Location of the shader normal matrix. normal_matrix =
    ...
    int create(const char *vs,
               const char *fs,
               bool has_tex); /* Create shader program from source. */
   ...
   private:
    static const char *const
        shader_col_vsource; /* Vertex shader source code for the default (non-textured) shader. */
    static const char *const shader_col_fsource; /* Fragment shader source code for the default
                                                    (non-textured) shader. */
    static const char
        *const shader_tex_vsource; /* Vertex shader source code for the textured shader. */
    static const char
        *const shader_tex_fsource; /* Fragment shader source code for the textured shader. */
  } Shader;
  ...
  class Texture {
   public:
    ...
    bool create_implementation(); /* Internal helper function to generate the implementation
                                     object. */
   public:
  ...
  typedef struct Model {
   ...
   public:
    ...
    static Model *create(float *verts,
                         float *nrmls,
                         float *uvs,
                         uint num_verts); /* Create model from buffers. */
 ...
 public:
  ...
  static int create_controller_models(
      VR_UI_Type type); /* Create the controller models for a specified UI type. */
 ...
 public:
  ...
  static void update_projection_matrix(
      const float _projection[4][4]); /* Set the current projection matrix. */
  static void update_modelview_matrix(const Mat44f *_model,
                                      const Mat44f *_view); /* Set the current modelview matrix. */
  static void set_color(const float color[4]); /* Set the current object / render color. */
  static void set_color(const float &r,
                        const float &g,
                        const float &b,
                        const float &a); /* Set the current object / render color. */
  ...
  static void render_rect(
      ...
      float u = 1.0f,
      float v = 1.0f,
      Texture *tex = 0); /* Render a rectangle with currently set transformation. */
  static void render_frame(float left,
                           float right,
                           float top,
                           float bottom,
                           float b,
                           float z = 0); /* Render a flat frame. */
  static void render_box(const Coord3Df &p0,
                         const Coord3Df &p1,
                         bool outline = false); /* Render an axis-aligned box. */
  static void render_ball(
      float r, bool golf = false); /* Render a ball with currently set transformation. */
  static void render_arrow(const Coord3Df &from,
                           const Coord3Df &to,
                           float width); /* Render an arrow. */
  static void render_string(const char *str,
                            ...
                            float x_offset = 0.0f,
                            float y_offset = 0.0f,
                            float z_offset = 0.0f); /* Render a string. */
};
