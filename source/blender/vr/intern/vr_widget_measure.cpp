...
void Widget_Measure::draw_line(VR_UI::Cursor &c, Coord3Df &localP0, Coord3Df &localP1)
{
  ...
  current_stroke = BKE_gpencil_add_stroke(
      Widget_Annotate::gpf[active_layer], 0, 3, line_thickness * 1.6f);
...
void Widget_Measure::render(VR_Side side)
{
  ...
  if (measure_ctrl_state == VR_UI::CTRLSTATE_OFF) {
    ...
    VR_Draw::render_string(
        measure_str.c_str(), 0.02f, 0.02f, VR_HALIGN_CENTER, VR_VALIGN_TOP, 0.0f, 0.08f, 0.001f);
  }
  else {
    /* Angle measurement */
