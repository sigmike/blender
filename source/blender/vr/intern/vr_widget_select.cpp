...
static void raycast_select_multiple(const float &x0,
                                    const float &y0,
                                    const float &x1,
                                    const float &y1,
                                    bool extend,
                                    ...
                                    bool obcenter = true)
{
  ...
  const eObjectMode object_mode = oldbasact ? (eObjectMode)oldbasact->object->mode :
                                              OB_MODE_OBJECT;
  bool is_obedit;
  // int hits;
  ...
  if (BASACT(view_layer) && BASACT(view_layer)->next)
    startbase = BASACT(view_layer)->next;
  ...
  /* This block uses the control key to make the object selected by its center point rather than
   * its contents */
  ...
  if (obcenter) {
    ...
    if (enumerate) {
      ...
      const int object_type_exclude_select = (vc.v3d->object_type_exclude_viewport |
                                              vc.v3d->object_type_exclude_select);
      base = startbase;
      while (base) {
        if (BASE_SELECTABLE(v3d, base) &&
...
/* Adapted from view3d_select.c */
static void raycast_select_multiple_vertex(const float &x0,
                                           const float &y0,
                                           const float &x1,
                                           const float &y1,
                                           ViewContext *vc,
  ...
  BM_ITER_MESH (v, &iter, bm, BM_VERTS_OF_MESH) {
    if (!BM_elem_flag_test(v, BM_ELEM_HIDDEN)) {
      ...
      if (VR_Util::view3d_project(ar,
                                  ...
                                  (float *)&pos,
                                  screen_co,
                                  (eV3DProjTest)(V3D_PROJ_TEST_CLIP_BB |
        ...
        if (fabsf(screen_co[0] - center_x) < bounds_x &&
          ...
          const int sel_op_result = ED_select_op_action_deselected(
              deselect ? SEL_OP_SUB : SEL_OP_ADD, is_select, is_inside);
          if (sel_op_result != -1) {
...
static void raycast_select_multiple_edge(const float &x0,
                                         const float &y0,
                                         const float &x1,
                                         const float &y1,
                                         ViewContext *vc,
...
static void raycast_select_multiple_face(const float &x0,
                                         const float &y0,
                                         const float &x1,
                                         const float &y1,
                                         ViewContext *vc,
...
static void raycast_select_multiple_edit(const float &x0,
                                         const float &y0,
                                         const float &x1,
                                         const float &y1,
                                         bool extend,
...
void Widget_Select::Raycast::drag_start(VR_UI::Cursor &c)
{
  ...
  VR_UI::get_screen_coordinates(
      *(Coord3Df *)m.m[3], selection_rect[side].x0, selection_rect[side].y0, side);
...
void Widget_Select::Raycast::drag_contd(VR_UI::Cursor &c)
{
  ...
  VR_Side side = VR_UI::eye_dominance_get();
  VR_UI::get_screen_coordinates(
      *(Coord3Df *)m.m[3], selection_rect[side].x1, selection_rect[side].y1, side);
  VR_UI::get_screen_coordinates(
      *(Coord3Df *)m_interaction.m[3], selection_rect[side].x0, selection_rect[side].y0, side);
...
void Widget_Select::Raycast::drag_stop(VR_UI::Cursor &c)
{
  ...
  if (CTX_data_edit_object(vr_get_obj()->ctx)) {
    raycast_select_multiple_edit(selection_rect[side].x0,
                                 selection_rect[side].y0,
                                 selection_rect[side].x1,
                                 selection_rect[side].y1,
                                 VR_UI::shift_key_get(),
                                 VR_UI::ctrl_key_get());
  }
  else {
    raycast_select_multiple(selection_rect[side].x0,
                            selection_rect[side].y0,
                            selection_rect[side].x1,
                            selection_rect[side].y1,
                            VR_UI::shift_key_get(),
...
void Widget_Select::Raycast::render(VR_Side side)
{
  ...
  VR_Draw::render_frame(Raycast::selection_rect[side].x0,
                        Raycast::selection_rect[side].x1,
                        Raycast::selection_rect[side].y1,
                        Raycast::selection_rect[side].y0,
                        0.005f);
...
static void proximity_select_multiple(const Coord3Df &p0,
                                      const Coord3Df &p1,
                                      bool extend,
                                      bool deselect,
                                      bool toggle = false,
    ...
    if (enumerate) {
      ...
      while (base) {
        if (BASE_SELECTABLE(v3d, base) &&
          ...
          if (fabs(ob_pos.x - center.x) < bounds_x && fabs(ob_pos.y - center.y) < bounds_y &&
...
static void proximity_select_multiple_vertex(
    const Coord3Df &p0, const Coord3Df &p1, ViewContext *vc, bool extend, bool deselect)
{
  bContext *C = vr_get_obj()->ctx;
  /* Find bounds and center of selection box. */
  float bounds_x = fabsf(p1.x - p0.x) / 2.0f;
  ...
  BM_ITER_MESH (v, &iter, bm, BM_VERTS_OF_MESH) {
    if (!BM_elem_flag_test(v, BM_ELEM_HIDDEN)) {
      VR_Math::multiply_mat44_coord3D(pos, offset, *(Coord3Df *)v->co);
      if (fabs(pos.x - center.x) < bounds_x && fabs(pos.y - center.y) < bounds_y &&
...
static void proximity_select_multiple_edit(const Coord3Df &p0,
                                           const Coord3Df &p1,
                                           bool extend,
                                           bool deselect,
                                           bool toggle = false,
