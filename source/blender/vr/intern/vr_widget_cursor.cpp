...
void Widget_Cursor::cursor_teleport()
{
  ...
  BKE_scene_cursor_to_mat4(&scene->cursor, m_blender.m);
  static Mat44f m_real = VR_Math::identity_f;
  memcpy(m_real.m[3],
         VR_UI::convert_space(m_blender, VR_SPACE_BLENDER, VR_SPACE_REAL).m[3],
         sizeof(float) * 3);
...
void Widget_Cursor::cursor_set_to_world_origin()
{
  ...
  BKE_scene_cursor_from_mat4(&scene->cursor, (float(*)[4])VR_Math::identity_f.m, false);
...
void Widget_Cursor::cursor_set_to_object_origin()
{
  ...
  *(Coord3Df *)center.m[3] /= num_objects;
  ...
  /* Update the Blender 3D cursor */
  Scene *scene = CTX_data_scene(C);
  BKE_scene_cursor_mat3_to_rot(&scene->cursor, rot, false);
  copy_v3_v3(scene->cursor.location, center.m[3]);
...
void Widget_Cursor::click(VR_UI::Cursor &c)
{
  ...
  bContext *C = vr_get_obj()->ctx;
  Scene *scene = CTX_data_scene(C);
  BKE_scene_cursor_from_mat4(&scene->cursor, (float(*)[4])m.m, false);
