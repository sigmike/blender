...
static int add_primitive_plane_exec(bContext *C, VR_UI::Cursor &c)
{
  ...
  if (VR_UI::ctrl_key_get()) {
    ...
    Scene *scene = CTX_data_scene(C);
    float m[4][4];
    BKE_scene_cursor_to_mat4(&scene->cursor, m);
    mat4_to_eul(rot, m);
...
static int add_primitive_cone_exec(bContext *C, VR_UI::Cursor &c)
{
  ...
  if (!EDBM_op_call_and_selectf(em,
                                ...
                                "cap_tris=%b depth=%f matrix=%m4 calc_uvs=%b",
                                Widget_AddPrimitive::circle_vertices,
                                Widget_AddPrimitive::cone_radius1,
                                Widget_AddPrimitive::cone_radius2,
                                cap_end,
                                cap_tri,
                                Widget_AddPrimitive::depth,
                                creation_data.mat,
...
static int add_primitive_uvsphere_exec(bContext *C, VR_UI::Cursor &c)
{
  ...
  if (!EDBM_op_call_and_selectf(
          ...
          Widget_AddPrimitive::sphere_segments,
          Widget_AddPrimitive::sphere_rings,
          Widget_AddPrimitive::radius,
          creation_data.mat,
          Widget_AddPrimitive::calc_uvs)) {
    return OPERATOR_CANCELLED;
