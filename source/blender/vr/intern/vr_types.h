...
extern bool mat44_inverse(float inv[4][4],
                          const float m[4][4]); /* Fast matrix inversion using Eigen. */
extern bool mat44_inverse(double inv[4][4],
                          const double m[4][4]); /* Fast matrix inversion using Eigen. */
extern void mat44_multiply(
    float R[4][4],
    const float A[4][4],
    const float B[4][4]); /* Fast matrix multiplication using SSE2 operations. */
extern void mat44_multiply(
    double R[4][4],
    const double A[4][4],
    const double B[4][4]); /* Fast matrix multiplication using SSE2 operations. */
...
template<typename T = float> struct Mat44 {
  ...
  T det() const
  {
    return m[0][0] * m[1][1] * m[2][2] * m[3][3] + m[0][0] * m[1][2] * m[2][3] * m[3][1] +
...
\**************************************************************************************************/
extern ui64
    VR_t_now; /* Current (most recent) timestamp. This will be updated (1) when updating tracking
                 (2) when starting rendering a new frame (3) before executing UI operations. */
