...
void Widget_Annotate::render(VR_Side side)
{
  ...
  if (tot_points <= 1) {
    ...
    for (int i = 0; i < tot_points; ++i) {
      /* if there was a significant pressure change, stop the curve, change the thickness of the
       * stroke, and continue drawing again (since line-width cannot change in middle of
       * GL_LINE_STRIP) Note: we want more visible levels of pressures when thickness is bigger.
       */
      if (fabsf(points[i].pressure - cur_pressure) > 0.2f / (float)line_thickness) {
        /* if the pressure changes before get at least 2 vertices, need to repeat last point to
         * avoid assert in immEnd() */
        if (draw_points < 2) {
