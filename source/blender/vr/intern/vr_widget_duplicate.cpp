    ...
    if (dupflag & USER_DUP_MAT) {
      for (a = 0; a < obn->totcol; a++) {
        ...
        if (id) {
          ...
          {
            obn->mat[a] = (Material *)ID_NEW_SET(obn->mat[a],
                                                 BKE_material_copy(bmain, obn->mat[a]));
    ...
    if (dupflag & USER_DUP_PSYS) {
      ...
      for (psys = (ParticleSystem *)obn->particlesystem.first; psys; psys = psys->next) {
        ...
        if (id) {
          ...
          {
            psys->part = (ParticleSettings *)ID_NEW_SET(
    ...
    switch (obn->type) {
      ...
      case OB_LAMP:
        if (dupflag & USER_DUP_LAMP) {
          ...
          {
            obn->data = ID_NEW_SET(obn->data, BKE_light_copy(bmain, (const Light *)obn->data));
            didit = 1;
          }
          id_us_min(id);
        }
      ...
      case OB_ARMATURE:
        ...
        if (dupflag & USER_DUP_ARM) {
          ...
          {
            obn->data = ID_NEW_SET(obn->data,
                                   BKE_armature_copy(bmain, (const bArmature *)obn->data));
        ...
        if (matarar) {
          for (a = 0; a < obn->totcol; a++) {
            ...
            if (id) {
              ...
              {
                (*matarar)[a] = (Material *)ID_NEW_SET((*matarar)[a],
                                                       BKE_material_copy(bmain, (*matarar)[a]));
              }
              id_us_min(id);
            }
          }
...
static int edbm_duplicate_exec(bContext *C, wmOperator *op)
{
  ...
  for (uint ob_index = 0; ob_index < objects_len; ob_index++) {
    ...
    EDBM_op_init(
        em, &bmop, op, "duplicate geom=%hvef use_select_history=%b", BM_ELEM_SELECT, true);
...
void Widget_Duplicate::render_icon(const Mat44f &t,
