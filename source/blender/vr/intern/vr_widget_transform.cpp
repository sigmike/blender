...
Widget_Transform::TransformMode Widget_Transform::transform_mode(
    Widget_Transform::TRANSFORMMODE_OMNI);
...
/* Manipulator colors. */
static const float c_manip[4][4] = {1.0f,
                                    0.2f,
                                    0.322f,
                                    0.4f,
                                    0.545f,
                                    ...
                                    1.0f,
                                    0.4f};
static const float c_manip_select[4][4] = {1.0f,
                                           0.2f,
                                           0.322f,
                                           1.0f,
                                           0.545f,
...
void Widget_Transform::raycast_select_manipulator(const Coord3Df &p, bool *extrude)
{
  ...
  for (int i = 0; i < 16; ++i) {
    switch (i) {
      ...
      case 12: { /* xy plane */
        if (omni ||
            (transform_mode != TRANSFORMMODE_MOVE && transform_mode != TRANSFORMMODE_SCALE)) {
          i += 2;
          continue;
        };
        pos = manip_pos +
              (axis[0] * axis_length[0] + axis[1] * axis_length[1]) * manip_scale_factor / 2.0f;
        break;
      }
      case 13: { /* yz plane */
        pos = manip_pos +
              (axis[1] * axis_length[1] + axis[2] * axis_length[2]) * manip_scale_factor / 2.0f;
        break;
      }
      case 14: { /* zx plane */
        pos = manip_pos +
              (axis[0] * axis_length[0] + axis[2] * axis_length[2]) * manip_scale_factor / 2.0f;
        break;
      }
      case 15: { /* center box */
        if (!omni) {
    ...
    if (VR_Util::view3d_project(ar,
                                rv3d->persmat,
                                false,
                                (float *)&pos,
                                screen_co,
                                (eV3DProjTest)(V3D_PROJ_TEST_CLIP_BB | V3D_PROJ_TEST_CLIP_NEAR)) ==
        V3D_PROJ_RET_OK) {
...
void Widget_Transform::drag_contd(VR_UI::Cursor &c)
{
  ...
  if (c.bimanual) {
    ...
    prev.m[0][0] = x_axis_prev.x;
    prev.m[0][1] = x_axis_prev.y;
    prev.m[0][2] = x_axis_prev.z;
    prev.m[1][0] = y_axis_prev.x;
    prev.m[1][1] = y_axis_prev.y;
    prev.m[1][2] = y_axis_prev.z;
    prev.m[2][0] = z_axis_prev.x;
    prev.m[2][1] = z_axis_prev.y;
    prev.m[2][2] = z_axis_prev.z;
    curr.m[0][0] = x_axis_curr.x;
    curr.m[0][1] = x_axis_curr.y;
    curr.m[0][2] = x_axis_curr.z;
    curr.m[1][0] = y_axis_curr.x;
    curr.m[1][1] = y_axis_curr.y;
    curr.m[1][2] = y_axis_curr.z;
    curr.m[2][0] = z_axis_curr.x;
    curr.m[2][1] = z_axis_curr.y;
    curr.m[2][2] = z_axis_curr.z;

    /* Translation: translation of the averaged pointer positions */
    prev.m[3][0] = (prev_h.m[3][0] + prev_o.m[3][0]) / 2.0f;
    prev.m[3][1] = (prev_h.m[3][1] + prev_o.m[3][1]) / 2.0f;
    prev.m[3][2] = (prev_h.m[3][2] + prev_o.m[3][2]) / 2.0f;
    prev.m[3][3] = 1;
    curr.m[3][0] = (curr_h.m[3][0] + curr_o.m[3][0]) / 2.0f;
    curr.m[3][1] = (curr_h.m[3][1] + curr_o.m[3][1]) / 2.0f;
    curr.m[3][2] = (curr_h.m[3][2] + curr_o.m[3][2]) / 2.0f;
    curr.m[3][3] = 1;

    if (transform_mode != TRANSFORMMODE_ROTATE) {
      /* Scaling: distance between pointers */
      float curr_s = sqrt(((curr_h.m[3][0] - curr_o.m[3][0]) * (curr_h.m[3][0] - curr_o.m[3][0])) +
                          ((curr_h.m[3][1]) - curr_o.m[3][1]) *
                              (curr_h.m[3][1] - curr_o.m[3][1])) +
                     ((curr_h.m[3][2] - curr_o.m[3][2]) * (curr_h.m[3][2] - curr_o.m[3][2]));
      float start_s =
          sqrt(((prev_h.m[3][0] - prev_o.m[3][0]) * (prev_h.m[3][0] - prev_o.m[3][0])) +
               ((prev_h.m[3][1]) - prev_o.m[3][1]) * (prev_h.m[3][1] - prev_o.m[3][1])) +
          ((prev_h.m[3][2] - prev_o.m[3][2]) * (prev_h.m[3][2] - prev_o.m[3][2]));
      ...
      switch (constraint_mode) {
        ...
        case VR_UI::CONSTRAINTMODE_SCALE_XY: {
          ...
          if (c.bimanual) {
            ...
            *(Coord3Df *)scale = ((*(Coord3Df *)manip_t_orig.m[0]).normalize() +
                                  (*(Coord3Df *)manip_t_orig.m[1]).normalize()) /
                                 2.0f;
          }
          else {
            project_v3_v3v3(&temp1.x, delta_orig.m[3], manip_t_orig.m[0]);
        ...
        case VR_UI::CONSTRAINTMODE_SCALE_YZ: {
          ...
          if (c.bimanual) {
            ...
            *(Coord3Df *)scale = ((*(Coord3Df *)manip_t_orig.m[1]).normalize() +
                                  (*(Coord3Df *)manip_t_orig.m[2]).normalize()) /
                                 2.0f;
          }
          else {
            project_v3_v3v3(&temp1.x, delta_orig.m[3], manip_t_orig.m[1]);
      ...
      if (obedit) { /* Edit mode */
        ...
        switch (snap_mode) {
          ...
          case VR_UI::SNAPMODE_SCALE: {
            /* Scale */
            /* TODO_XR */
            // if (transform_space == VR_UI::TRANSFORMSPACE_GLOBAL && constraint_mode !=
            // VR_UI::CONSTRAINTMODE_NONE) {
            //	/* TODO_XR */
            //	break;
            //	/*for (int i = 0; i < 3; ++i) {
            //		if (snap_flag[i]) {
            //			continue;
            //		}
            //		(*(Coord3Df*)manip_t_snap.m[i]).normalize_in_place() *=
            //(*(Coord3Df*)nonsnap_m.m[i]).length();
            //	}
            //	static Mat44f t;
        ...
        switch (snap_mode) {
          ...
          case VR_UI::SNAPMODE_SCALE: {
            /* Scale */
            if (transform_space == VR_UI::TRANSFORMSPACE_GLOBAL &&
                constraint_mode != VR_UI::CONSTRAINTMODE_NONE) {
              /* TODO_XR */
              break;
              /*for (int i = 0; i < 3; ++i) {
                ...
                (*(Coord3Df*)obmat.m[i]).normalize_in_place() *=
              (*(Coord3Df*)nonsnap_m.m[i]).length();
              }
              static Mat44f t;
              transpose_m4_m4(t.m, nonsnap_m.m);
          ...
          case TRANSFORMMODE_SCALE: {
            if (transform_space == VR_UI::TRANSFORMSPACE_LOCAL &&
              ...
              for (int i = 0; i < 3; ++i) {
                ...
                (*(Coord3Df *)obact->obmat[i]).normalize_in_place() *=
                    (*(Coord3Df *)m.m[i]).length();
              }
            }
            else {
              for (int i = 0; i < 3; ++i) {
...
void Widget_Transform::drag_stop(VR_UI::Cursor &c)
{
  ...
  if (c.bimanual) {
    ...
    c.other_hand->interaction_position.set(
        ((Mat44f)VR_UI::cursor_position_get(VR_SPACE_REAL, other->side)).m, VR_SPACE_REAL);
    ...
    case 2: { /* Ball */
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        switch (i) {
          case 0: { /* x-axis */
            offset[0] = 0.0f;
            offset[1] = length[1] / 1.5f + scale / 2.0f;
            offset[2] = length[2] / 1.5f + scale / 2.0f;
            break;
          }
          case 1: { /* y-axis */
            offset[0] = length[0] / 1.5f + scale / 2.0f;
            offset[1] = 0.0f;
            offset[2] = length[2] / 1.5f + scale / 2.0f;
            break;
          }
...
void Widget_Transform::render_planes(const float length[3])
{
  ...
  for (int i = 0; i < 3; ++i) {
    len = length[i] / 4.0f;
    len2 = len / 8.0f;
    verts_plane[0][0] = -len2;
    verts_plane[0][1] = -len2;
    verts_plane[1][0] = len2;
    verts_plane[1][1] = -len2;
    verts_plane[2][0] = len2;
    verts_plane[2][1] = len2;
    verts_plane[3][0] = -len2;
    verts_plane[3][1] = len2;
...
void Widget_Transform::render_gimbal(const float radius[3],
                                     const bool filled,
      ...
      if (filled) {
        ...
        if (arc_partial_angle == 0.0f) {
          ...
          imm_draw_circle_partial_wire_2d(
              pos, 0, 0, rad, WIDGET_TRANSFORM_DIAL_RESOLUTION, 0.0f, arc_partial_deg);
        }
      }
                               ...
                               const float radius)
{
  ...
  if (arc_inner_factor != 0.0) {
    ...
    imm_draw_disk_partial_fill_2d(pos,
                                  ...
                                  arc_inner_factor,
                                  width_inner,
                                  WIDGET_TRANSFORM_DIAL_RESOLUTION,
                                  RAD2DEGF(angle_ofs),
                                  RAD2DEGF(M_PI * 2));
  }
...
void Widget_Transform::render_dial(const float angle_ofs,
                                   const float angle_delta,
                                   const float arc_inner_factor,
                                   const float radius)
{
...
void Widget_Transform::render_incremental_angles(const float incremental_angle,
                                                 const float offset,
                                                 const float radius)
{
...
void Widget_Transform::render(VR_Side side)
{
  ...
  /* If, during the Widget_Transform::update_manipulator() call no object was selected, we
     zeroed-out the manip_t matrix.
     For  better performance and to avoid potential issues (eg. when trying to calculate inverse),
        ...
        case VR_UI::CONSTRAINTMODE_ROT_X: {
          ...
          if (VR_UI::ctrl_key_get()) {
            if (VR_UI::shift_key_get()) {
              ...
              render_incremental_angles(
                  WIDGET_TRANSFORM_ROT_PRECISION, 0.0f, manip_length[0] / 4.0f);
            }
          }
          GPU_matrix_rotate_axis(90.0f, 'Y');
          break;
        ...
        case VR_UI::CONSTRAINTMODE_ROT_Y: {
          ...
          if (VR_UI::ctrl_key_get()) {
            if (VR_UI::shift_key_get()) {
              ...
              render_incremental_angles(
                  WIDGET_TRANSFORM_ROT_PRECISION, 0.0f, manip_length[1] / 4.0f);
            }
          }
          GPU_matrix_rotate_axis(-90.0f, 'X');
          break;
        ...
        case VR_UI::CONSTRAINTMODE_ROT_Z: {
          ...
          if (VR_UI::ctrl_key_get()) {
            if (VR_UI::shift_key_get()) {
              ...
              render_incremental_angles(
                  WIDGET_TRANSFORM_ROT_PRECISION, 0.0f, manip_length[2] / 4.0f);
            }
          }
          GPU_matrix_rotate_axis(90.0f, 'Z');
          break;
