...
static bool edbm_inset_init(bContext *C, wmOperator *op, const bool is_modal)
{
...

  {
    ...
    Object **objects = BKE_view_layer_array_from_objects_in_mode_params(
        view_layer, CTX_wm_view3d(C), &ob_store_len, &params);

    opdata->ob_store = (InsetObjectStore *)MEM_malloc_arrayN(
        ob_store_len, sizeof(*opdata->ob_store), __func__);
...
static bool edbm_inset_calc(wmOperator *op)
{
  ...
  const bool use_select_inset =
      false;  // RNA_boolean_get(op->ptr, "use_select_inset"); /* not passed onto the BMO */
  ...
  for (uint ob_index = 0; ob_index < opdata->ob_store_len; ob_index++) {
    ...
    if (Widget_InsetFaces::use_individual) {
      EDBM_op_init(em,
                   ...
                   BM_ELEM_SELECT,
                   Widget_InsetFaces::use_even_offset,
                   Widget_InsetFaces::use_relative_offset,
                   use_interpolate,
                   Widget_InsetFaces::thickness,
                   Widget_InsetFaces::depth);
    }
    else {
      EDBM_op_init(
          em,
          ...
          BM_ELEM_SELECT,
          Widget_InsetFaces::use_boundary,
          Widget_InsetFaces::use_even_offset,
          Widget_InsetFaces::use_relative_offset,
          use_interpolate,
          Widget_InsetFaces::thickness,
          Widget_InsetFaces::depth,
          Widget_InsetFaces::use_outset,
          use_edge_rail);
