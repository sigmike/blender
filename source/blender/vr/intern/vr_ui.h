...
// Default distance threshold (meters) to detect "dragging".
#define VR_UI_DEFAULTDRAGTHRESDIST 0.012f

// Default rotation threshold (deg) to detect "dragging".
#define VR_UI_DEFAULTDRAGTHRESROT 8.0f

// Default time threshold (ms) to distinguish between "clicking" and "dragging".
#define VR_UI_DEFAULTDRAGTHRESTIME 150

// Size of the estimated default workspace in meters.
#define VR_UI_DEFAULTWORKSPACESIZE 0.450f

// Distance of default workspace center from the HMD in meters.
#define VR_UI_DEFAULTWORKSPACEDIST 0.550f

// Height of default workspace center (relative to HMD) in meters.
#define VR_UI_DEFAULTWORKSPACEHEIGHT -0.350f

// Maximum interval in ms at which the UI should perform updates on Blender. (ie. minimum refresh
// rate: 1Hz)
#define VR_UI_MAXUPDATEINTERVAL (1000 / 1)

// Minimum interval in ms at which the UI should perform updates on Blender. (ie. max refresh rate:
// 60Hz)
#define VR_UI_MINUPDATEINTERVAL (1000 / 120)

// Minimum navigation scale (Real to Blender) permitted.
#define VR_UI_MINNAVIGATIONSCALE 0.001f

// Maximum navigation scale (Real to Blender) permitted.
#define VR_UI_MAXNAVIGATIONSCALE 1000.0f
...
class VR_UI {
 ...
 public:
  ...
  typedef struct LMatrix2 {
    ...
    const Mat44f &get(VR_Space s = VR_SPACE_REAL,
                      bool inverse = false); /* Retrieve the matrix in a given space. */
  } LMatrix2;
  ...
  typedef struct Cursor {
    ...
    bool
        active; /* Whether this cursor is active (i.e. used, not necessarily clicking anything). */
    ...
    CtrlState interaction_ctrl;    /* Whether the CTRL key was pressed when the interaction was
                                      started (0 or 1). */
    ShiftState interaction_shift;  /* Whether the SHIFT key was pressed when the interaction was
                                      started (0 or 1). */
    AltState interaction_alt; /* Whether the ALT key was pressed when the interaction was started
                                 (0 or 1). */
    VR_Widget *interaction_widget; /* Currently active widget (or 0 if none is mapped). */
    Coord3Df offset_pos; /* Relative positional offset between actualy controller position and
                            virtual cursor position (global). */
    Mat44f offset_rot; /* Relative rotational offset between actual controller rotation and virtual
                          cursor rotation (local). */
 ...
 protected:
  ...
  static float navigation_scale;    /* Scale factor for the Blender origin (origin_m * scale =
                                       blender_space_units). */

  static bool hmd_position_current[VR_SPACES][2]; /* Whether the HMD transformation is current for
                                                     [VR_SPACE][inverse] */
  static bool eye_position_current[VR_SPACES][VR_SIDES]
                                  [2]; /* Whether the eye transformation is current for
                                          [VR_SPACE][VR_SIDE][inverse] */
  ...
  static bool controller_position_current[VR_SPACES][VR_MAX_CONTROLLERS]
                                         [2]; /* Whether the controller transformation is current
                                                 for [VR_SPACE][VR_SIDE][inverse] */

  static Cursor cursor[VR_MAX_CONTROLLERS]; /* Cursors for both hands plus auxilliary controllers
                                               if available. */
 ...
 public:
  ...
  static const float &navigation_scale_get();    /* Scale factor for the Blender origin (origin_m *
                                                    scale = blender_space_units). */
  static void navigation_set(const Mat44f &m);   /* Set navigation matrix. */
  static void navigation_apply_transformation(
      const Mat44f &m,
      VR_Space space = VR_SPACE_BLENDER,
      bool inverse =
          false); /* Apply navigational transformation, relative to current navigation. */
  static void navigation_reset(); /* Reset navigation matrix to identity. */

  static void navigation_fit_scene(); /* Reset navigation to fit the whole scene into the work
                                         area. */
  static void navigation_fit_selection(
      VR_Direction look_from_direction = VR_DIRECTION_NONE); /* Reset navigation to fit the current
                                                                selection into the work area. */

  static bool is_zaxis_up(); /* Whether the z-axis is the up direction in Blender. (If not, the
                                y-axis is assumed to be the up direction). */
  static void navigation_orient_up(
      Coord3Df *pivot =
          0); /* Rotate the origin so than the Blender up-axis is facing (real) upwards again. */

  static NavMode navigation_mode; /* Current mode of navigation (locomotion). */

  static const Mat44f &hmd_position_get(
      VR_Space space,
      bool inverse = false); /* Get the current transformation matrix of the HMD. */
  static const Mat44f &eye_position_get(
      VR_Space space,
      VR_Side side,
      bool inverse =
          false); /* Get the transformation matrix of the eye (or physical camera in space). */
  ...
  static void eye_baseline_set(
      float baseline); /* Set the distance between the eyes in real-world meters. */
  ...
  static void hand_dominance_set(
      VR_Side side); /* Set dominant hand side (right-handed or left-handed). */

  static const Mat44f &controller_position_get(
      VR_Space space,
      VR_Side side,
      bool inverse = false); /* Get the transformation matrix of the controller. */

  static const Mat44f &cursor_position_get(
      VR_Space space,
      VR_Side side,
      bool inverse = false); /* Get the 3D cursor position of either hand. */
  static const Mat44f &cursor_interaction_position_get(
      VR_Space space,
      VR_Side side,
      bool inverse = false); /* Get the 3D cursor position at the time of interaction start. */
  static ui64 cursor_buttons_get(VR_Side side); /* Get the cursors current button state. */
  static bool cursor_trigger_get(
      VR_Side side); /* Get whether the trigger button is currently pressed on the controller. */
  static bool cursor_grip_get(
      VR_Side side); /* Get whether the grip button is currently pressed on the controller. */
  static bool cursor_active_get(
      VR_Side side); /* Get whether the 3D cursor is currently active (in use, but not necessarily
                        doing anything right now). */
  static bool cursor_visible_get(VR_Side side); /* Get whether the 3D cursor is set to be visible
                                                   (tracked, but not necessarily on screen). */
  static void cursor_position_set(
      VR_Space space,
      VR_Side side,
      const Mat44f &m); /* Set the 3D cursors current transformation. */
  static void cursor_active_set(VR_Side side, bool b); /* Set whether the 3D cursor is active. */
  static void cursor_visible_set(VR_Side side,
                                 bool b); /* Set whether the 3D cursor should be rendered. */
  static bool
      cursor_offset_enabled;        /* Whether the controller offset is currently being enabled. */
  static bool cursor_offset_update; /* Whether the controller offset is currently being updated. */
  static void cursor_offset_set(
      VR_Side side,
      const Mat44f &rot,
      const Coord3Df &pos); /* Set the current cursor offset for target cursor. */
  ...
  static CtrlState ctrl_key_get();   /* Get Whether the CTRL key is currently held down (on either
                                        controller). */
  static ShiftState shift_key_get(); /* Get whether the SHIFT key is currently held down (on either
                                        controller). */
  static AltState alt_key_get();     /* Get whether the ALT key is currently held down (on either
                                        controller). */
  ...
  static VR_Widget *get_current_tool(
      VR_Side side); /* Get the currently active tool for the controller. */
  static VR_UI::Error set_current_tool(
      const VR_Widget *tool, VR_Side side); /* Set the currently active tool for the controller. */

  static Mat44f convert_space(const Mat44f &m,
                              VR_Space m_space,
                              VR_Space target_space);  //!< Convert a matrix into target space. */
  static Coord3Df convert_space(
      const Coord3Df &v,
      VR_Space v_space,
      VR_Space target_space);  //!< Convert vector/position into target space. */
  static int get_screen_coordinates(
      const Coord3Df &c,
      float &x,
      float &,
      VR_Side side = VR_SIDE_DOMINANT); /* Get 2D screen coordinates (-1 ~ 1) of a 3D point. */
  static int get_pixel_coordinates(
      const Coord3Df &c,
      int &x,
      int &y,
      VR_Side side = VR_SIDE_DOMINANT); /* Get 2D pixel coordinates of a 3D point. */

  static float scene_unit_scale(
      VR_Space space); /* Get the length (scale) of Blender scene units in respective space. */

  static float drag_threshold_distance; /* Distance threshold (meters) to detect "dragging" (if the
                                           cursor moves farther than this with a button held down,
                                           it's dragging). */
  static float drag_threshold_rotation; /* Rotation threshold (deg) to detect "dragging" (if the
                                           cursor rotates more than this with a button held down,
                                           it's dragging). */
  static uint drag_threshold_time; /* Time threshold (ms) to distinguish between "clicking" and
                                      "dragging". */

  static int
      undo_count; /* Number of pending VR_UI undo operations to be executed post-scene render. */
  static int
      redo_count; /* Number of pending VR_UI redo operations to be executed post-scene render. */
  ...
  static const VR_Widget
      *pie_menu[VR_SIDES]; /* The current VR pie menu for the controller (if any). */
 ...
 public:
  ...
  static Error update_tracking();    /* Update HMD and cursor positions, button state, etc.
                                        (pre-render). */
  static Error execute_operations(); /* Execute internal UI operations and possibly execute users
                                        actions in Blender. (pre-render). */
  static Error update_cursor(Cursor &c);         /* Cursor interaction update. */
  static Error update_menus();                   /* Update any VR floating menus. */
  static Error execute_post_render_operations(); /* Execute special UI operations (i.e. undo/redo)
                                                    that need to be called after the scene is
                                                    rendered. */

  static Error pre_render(
      VR_Side side); /* Render UI elements, called prior to rendering the scene. */
  static Error post_render(
      VR_Side side); /* Render UI elements, called after rendering the scene. */

  static Error render_controller(
      VR_Side controller_side); /* Helper function to render the controller. */
  static Error render_widget_icons(
      VR_Side controller_side,
      const Mat44f
          &t_controller); /* Helper function to render the widget icons on the controller. */

  static Error execute_widget_renders(
      VR_Side side); /* Helper function to execute widget render functions. */
  static Error render_menus(
      const Mat44f *_model,
      const Mat44f *_view); /* Helper function to render VR floating menus. */
};
