...
template<typename T = float> struct Quat {
  ...
  Quat(const Coord3Df &from, const Coord3Df &to)
  { /* TODO_XR: Use a generic Coord3D */
    Coord3Df v = from ^ to;
    x = v.x;
    y = v.y;
    z = v.z;
  ...
  Quat(const Coord3Df &axis, const T &angle)
  { /* TODO_XR: Use a generic Coord3D */
    T s = sin(angle / (T)2);
    T c = cos(angle / (T)2);
    x = axis.x * s;
  ...
  /* "Axis,angle" function */
  void to_axis_angle(Coord3Df &axis, T &angle)
  { /* TODO_XR: Use a generic Coord3D */
    if (w > (T)1) {
  ...
  Coord3Df operator*(const Coord3Df &c) const
  { /* TODO_XR: Use a generic Coord3D */
    T n0 = x * (T)2;
    T n1 = y * (T)2;
    T n2 = z * (T)2;
...
class VR_Math {
 public:
  ...
  static void multiply_mat44_coord3D(
      Coord3Df &r,
      const Mat44f &m,
      const Coord3Df &v); /* Multiply a Coord3Df by a Mat44f (drops the w component). */
  static float matrix_distance(
      const Mat44f &a,
      const Mat44f &b); /* Extract the distance between two Mat44f transformations. */
  static float matrix_rotation(
      const Mat44f &a,
      const Mat44f &b,
      Coord3Df *axis = 0); /* Get the relative rotation bewteen two transformation matrices. */
  static void orient_matrix_z(
      Mat44f &m, Coord3Df z); /* Rotate transformation matrix to align it with given z-vector. */
};
