...
typedef int(__stdcall *c_createVR)(
    void); /* Create the internal VR object. Must be called before the functions below. */
#ifdef WIN32
typedef int(__stdcall *c_initVR)(void *device,
                                 void *context); /*Initialize the internal VR object (OpenGL). */
#else
typedef int(__stdcall *c_initVR)(void *display,
                                 void *drawable,
                                 void *context); /* Initialize the internal object (OpenGL). */
#endif
typedef int(__stdcall *c_getHMDType)(int *type); /* Get the type of HMD used for VR. */
typedef int(__stdcall *c_setEyeParams)(
    int side, float fx, float fy, float cx, float cy); /* Set rendering parameters. */
typedef int(__stdcall *c_getDefaultEyeParams)(
    int side, float *fx, float *fy, float *cx, float *cy); /* Get the HMD's default parameters. */
typedef int(__stdcall *c_getDefaultEyeTexSize)(int *w,
                                               int *h,
                                               int side); /* Get the default eye texture size. */
typedef int(__stdcall *c_updateTrackingVR)(
    void); /* Update the t_eye positions based on latest tracking data. */
typedef int(__stdcall *c_getEyePositions)(
    float t_eye[VR_SIDES][4][4]); /* Last tracked position of the eyes. */
typedef int(__stdcall *c_getHMDPosition)(
    float t_hmd[4][4]); /* Last tracked position of the HMD. */
typedef int(__stdcall *c_getControllerPositions)(
    float t_controller[VR_MAX_CONTROLLERS][4][4]); /* Last tracked position of the controllers. */
typedef int(__stdcall *c_getControllerStates)(
    void *controller_states[VR_MAX_CONTROLLERS]); /* Last tracked button states of the controllers.
                                                   */
typedef int(__stdcall *c_blitEye)(
    int side,
    void *texture_resource,
    const float *aperture_u,
    const float *aperture_v); /* Blit a rendered image into the internal eye texture. */
typedef int(__stdcall *c_blitEyes)(
    void *texture_resource_left,
    void *texture_resource_right,
    const float *aperture_u,
    const float *aperture_v); /* Blit rendered images into the internal eye textures. */
...
static int vr_load_dll_functions(void)
{
  ...
  vr_dll_get_default_eye_params = (c_getDefaultEyeParams)GetProcAddress(vr_dll,
                                                                        "c_getDefaultEyeParams");
  if (!vr_dll_get_default_eye_params) {
    return -1;
  }
  vr_dll_get_default_eye_tex_size = (c_getDefaultEyeTexSize)GetProcAddress(
      vr_dll, "c_getDefaultEyeTexSize");
  ...
  vr_dll_get_controller_positions = (c_getControllerPositions)GetProcAddress(
      vr_dll, "c_getControllerPositions");
  if (!vr_dll_get_controller_positions) {
    return -1;
  }
  vr_dll_get_controller_states = (c_getControllerStates)GetProcAddress(vr_dll,
                                                                       "c_getControllerStates");
  ...
  vr_dll_get_default_eye_tex_size = (c_getDefaultEyeTexSize)dlsym(vr_dll,
                                                                  "c_getDefaultEyeTexSize");
  if (!vr_dll_get_default_eye_tex_size) {
    return -1;
  }
  ...
  vr_dll_get_controller_positions = (c_getControllerPositions)dlsym(vr_dll,
                                                                    "c_getControllerPositions");
  if (!vr_dll_get_controller_positions) {
    return -1;
  }
  ...
  if (!ar->draw_buffer) {
    ...
    for (int i = 0; i < 2; ++i) {
      GPUOffScreen *offscreen = GPU_offscreen_create(
          vr.tex_width, vr.tex_height, 0, true, true, NULL);
      if (!offscreen) {
      ...
      vr.viewport[i] = ar->draw_buffer->viewport[i] = GPU_viewport_create_from_offscreen(
          vr.offscreen[i]);
    }
...
#if WITH_VR
  vr_dll_blit_eyes(
      (void *)(&(vr.viewport[VR_SIDE_LEFT]->fbl->default_fb->attachments[2].tex->bindcode)),
...
void vr_compute_viewplane(const View3D *v3d, CameraParams *params, int winx, int winy)
{
  ...
  params->clip_start = vr.clip_start;  // * navscale; Don't need to apply, because the scale factor
                                       // on the view matrix affects all transformations.
  ...
  /*
   * OVERRIDE: due to the navigation, the clipping distance (in Blender coordinates) may change
   * between frames. Some tools rely on the View3D having the correct clipping distances set, so we
   * have to override this value here.
   */
  {
