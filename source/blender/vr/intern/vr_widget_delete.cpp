...
static int delete_selected_objects(bool use_global = true)
{
  ...
  for (ctx_link = (CollectionPointerLink *)ctx_data_list.first; ctx_link;
    ...
    /* This is sort of a quick hack to address T51243 - Proper thing to do here would be to nuke
     * most of all this custom scene/object/base handling, and use generic lib remap/query for
     * that. But this is for later (aka 2.8, once layers & co are settled and working).
     */
    ...
    /* XXX This is likely not correct? Will also remove parent from grease pencil from other
     * scenes, even when use_global is false... */
    ...
    if (use_global) {
      Scene *scene_iter;
      for (scene_iter = (Scene *)bmain->scenes.first; scene_iter;
  ...
  for (uint ob_index = 0; ob_index < objects_len; ob_index++) {
    ...
    switch (type) {
      case MESH_DELETE_VERT: /* Erase Vertices */
        if (!(em->bm->totvertsel && EDBM_op_callf(em,
                                                  &delete_dummy_op,
                                                  "delete geom=%hv context=%i",
                                                  BM_ELEM_SELECT,
                                                  DEL_VERTS))) {
          continue;
        }
      ...
      case MESH_DELETE_EDGE: /* Erase Edges */
        if (!(em->bm->totedgesel && EDBM_op_callf(em,
                                                  &delete_dummy_op,
                                                  "delete geom=%he context=%i",
                                                  BM_ELEM_SELECT,
                                                  DEL_EDGES))) {
          continue;
        }
      ...
      case MESH_DELETE_FACE: /* Erase Faces */
        if (!(em->bm->totfacesel && EDBM_op_callf(em,
                                                  &delete_dummy_op,
...
void Widget_Delete::render_icon(const Mat44f &t,
