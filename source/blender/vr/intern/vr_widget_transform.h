...
class Widget_Transform : public VR_Widget {
  ...
  static bool omni; /* Whether the Transform tool is in omni mode (used for maintaining correct
                       transform_mode between interactions). */
  static VR_UI::ConstraintMode
      constraint_mode;              /* The current constraint mode for the Transform tool. */
  ...
  static std::vector<Mat44f *>
      nonsnap_t;       /* The actual (non-snapped) transformations of the interaction objects. */
  ...
  static VR_UI::TransformSpace
      transform_space;     /* The current transform space for the Transform tool. */
  ...
  static Coord3Df manip_angle[VR_UI::TRANSFORMSPACES]; /* The current manipulator angle (euler xyz)
                                                          when constraining rotations. */
  static float manip_scale_factor; /* Scale factor for the manipulator (relative to longest
                                      selected object axis). */

  static Mat44f obmat_inv; /* The inverse of the selected object's transformation (edit mode). */

  static void raycast_select_manipulator(
      const Coord3Df &p,
      bool *extrude = 0); /* Select a manipulator component with raycast selection. */
 public:
  static void update_manipulator(); /* Update the manipulator transform. */
 protected:
  static void render_axes(const float length[3],
                          int draw_style = 0);      /* Render manipulator axes. */
  static void render_planes(const float length[3]); /* Render manipulator planes. */
  static void render_gimbal(const float radius[3],
                            const bool filled,
                            const float axis_modal_mat[4][4],
                            const float clip_plane[4],
                            const float arc_partial_angle,
                            const float arc_inner_factor); /* Render manipulator gimbal. */
  static void render_dial(const float angle_ofs,
                          const float angle_delta,
                          const float arc_inner_factor,
                          const float radius); /* Render manipulator dial. */
  static void render_incremental_angles(
      const float incremental_angle,
      const float offset,
      const float radius); /* Render manipulator incremental angles. */
 public:
  ...
  virtual void drag_start(
      VR_UI::Cursor &c) override; /* Start a drag/hold-motion with the index finger / trigger. */
  virtual void drag_contd(
      VR_UI::Cursor &c) override; /* Continue drag/hold with index finger / trigger. */
  virtual void drag_stop(
      VR_UI::Cursor &c) override; /* Stop drag/hold with index finger / trigger. */

  virtual void render(
      VR_Side side) override; /* Apply the widget's custom render function (if any). */
};
