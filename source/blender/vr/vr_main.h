...
/* Maximum number of controllers that can be simultaneously supported. */
#define VR_MAX_CONTROLLERS 3

/* Default near clip plane for the VR viewport (eye) cameras. (in real-world meters). */
#define VR_CLIP_NEAR 0.01f
/* Default far clip plane for the VR viewport (eye) cameras. (in real-world meters). */
#define VR_CLIP_FAR 100.0f

typedef enum VR_Space {
  VR_SPACE_REAL = 0 /* Real-world coordinates, units are meters, origin is dependent on VR device
                       set-up (usually on the floor). */
  ,
...
typedef struct VR_Controller {
  ...
  unsigned long long
      buttons_touched;    /* Buttons currently touched on the controller (if available). */
...
typedef struct VR {
  ...
  int initialized; /* Whether the base VR module was successfully initialized and currently active.
                    */
  int ui_initialized; /* Whether the VR UI module was successfully initialized and currently
                         active. */
  ...
  float
      cx[VR_SIDES]; /* Horizontal principal point, in "image-width" - units(0.5 = image center). */
  float
      cy[VR_SIDES]; /* Vertical principal point, in "image-height" - units(0.5 = image center). */
  ...
  float clip_start; /* Near clip plane. */
  ...
  struct VR_Controller
      *controller[VR_MAX_CONTROLLERS]; /* Controllers associated with the HMD device. */
  float t_controller[VR_SPACES][VR_MAX_CONTROLLERS][4]
                    [4]; /* Last tracked positions of the controllers. */
...
int vr_init(
    struct bContext *C); /* Initialize VR operations. Returns 0 on success, -1 on failure. */
...
void vr_draw_region_bind(struct ARegion *ar,
                         int side); /* Bind the VR offscreen buffer for rendering. */
...
void vr_do_interaction(
    void); /* Interaction update/execution where the VR module may alter scene data. */
void vr_do_post_render_interaction(
    void); /* Interaction update/execution for special operations (i.e. undo/redo) that need to be
...
void vr_update_view_matrix(
    int side, const float view[4][4]); /* Update the OpenGL view matrix for the VR module. */
void vr_update_projection_matrix(
    int side,
    const float projection[4][4]); /* Update the OpenGL projection matrix for the VR module. */

struct rcti;

void vr_update_viewport_bounds(
    const struct rcti *bounds); /* Update viewport (window) bounds for the VR module. */

struct View3D;
struct CameraParams;

/* Utility functions. */
void vr_compute_viewplane(const struct View3D *v3d,
                          struct CameraParams *params,
                          int winx,
                          int winy);                        /* Compute the VR camera viewplane. */
